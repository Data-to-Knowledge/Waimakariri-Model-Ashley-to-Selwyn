...
if __name__ == '__main__':
    ...
    if run_mt3d:
        for red in reductions:
            ...
            nc_description = 'a {} percent reduction on  all land with a gmp nload >= 5kg/ha in the waimakariri zone'.format(
                red)
            ...
            nc_description = 'a {} percent reduction on  all land with a gmp nload >= 15kg/ha in the waimakariri zone'.format(
                red)
    ...
    scen_paths = {}
    for red in reductions:
        scen_paths['dairy_red{}'.format(red)] = env.gw_met_data(
            r"mh_modeling\netcdfs_of_key_modeling_data\GMP_plus_{}per_dairy_ucn.nc".format(red))
        scen_paths['kgha_15_red{}'.format(red)] = env.gw_met_data(
            r"mh_modeling\netcdfs_of_key_modeling_data\GMP_plus_{}per_on_15kgha_ucn.nc".format(red))
        scen_paths['kgha_5_red{}'.format(red)] = env.gw_met_data(
            r"mh_modeling\netcdfs_of_key_modeling_data\GMP_plus_{}per_on_5kgha_ucn.nc".format(red))
